...
# Read initial data (Data from GV Portal & David Table (CC))
```{r}
...
# Rename columns to same format and delete the ones not needed (step don´t really needed, just for an easier way to follow)
```{r}
head(MatrizFinal)
head(DataCC)
...
#Combine/add CC data to the Main Data Table (GV2050)
```{r}
...
# Delete data without DAP and add equation
```{r}
...
#Two options for analyses: 
#1st using all DAP values but deleting the 0 (dead plants)
```{r}
...
#2nd (more conservative): using DAP values <25. The equation according to the paper was estimated only with DAP values in the range 1-25 cm, so higher values should not be considered because of risk of overestimaiton
```{r}
...
##ojo, estos son valores por individuo, no por hectarea
...
#tal vez convenga hacer todo de nuevo, con todos los datos e islas, así aumentamos el n y usamos la formula general de la literatura (también hay más edades) (casi 500 datos de CC). Revisar si están todos pasados
...
```
#MAYBE ALSO CHECK TRUNK CIRCUNFERENCE? hAMMAN 1979
```{r}
#**Kitayama equations**
#DryMass(WoodyParts)= 0.01540*(DAP^2)*1.60906 #r2 = 0.99
#DryMass(Foliage) = 0.01769*(DAP^2)*0.77946, #r2 = 0.76

data_height$KitDM<-((0.01540*((data_height$Est.DAP^2)^1.60906))+(0.01769*((data_height$Est.DAP^2)^0.77946)))
data_height$Altura<-data_height$Altura/100   
```

```{r}
df3<-subset(data_height,Tratamiento=="Cocoon")
df4<-subset(data_height,Tratamiento=="Waterboxx®_Hidrogel")
df6<-subset(data_height,Tratamiento=="Cocoon_Hidrogel")
df8<-subset(data_height,Tratamiento=="Control")
df11<-subset(data_height,Tratamiento=="Waterboxx®")
df12<-subset(data_height,Tratamiento=="Hidrogel")

df2<-as.data.frame(unique(df3[, c("Altura", "age","KitDM")]))
df5<-as.data.frame(unique(df4[, c("Altura", "age","KitDM")]))
df7<-as.data.frame(unique(df6[, c("Altura", "age","KitDM")]))
df9<-as.data.frame(unique(df8[, c("Altura", "age","KitDM")]))
df13<-as.data.frame(unique(df11[, c("Altura", "age","KitDM")]))
df14<-as.data.frame(unique(df12[, c("Altura", "age","KitDM")]))

library(plotly)


test_plotly <- with(df2, interp(Altura, age, KitDM, 
                                        duplicate = "mean"))
test_plotly2 <- with(df5, interp(Altura, age, KitDM, 
                                        duplicate = "mean"))
test_plotly3 <- with(df7, interp(Altura, age, KitDM, 
                                        duplicate = "mean"))
test_plotly4 <- with(df9, interp(Altura, age, KitDM, 
                                        duplicate = "mean"))
test_plotly5 <- with(df13, interp(Altura, age, KitDM, 
                                        duplicate = "mean"))
test_plotly6 <- with(df14, interp(Altura, age, KitDM, 
                                        duplicate = "mean"))

plot_ly(x = test_plotly$x, y = test_plotly$y, z = test_plotly$z,
        contours = list(
          x = list(show = T, color = "black", width = 1, highlightcolor = "#ff0000", start = 0, end = 17, size = 0.5),
...
```
#INCREASE OF dm WITH AGE, but treatments made that plant store mean levels of DM at some age. less propency to fall?
...
#logistic model of survival. Additive terms because interaction was too complex (not same level always) to converge
```{r}
library(lme4)
...
data_summary <- data_height %>%
  filter(age >= 0.5 & age <= 1.5 | age >= 3.5 & age <= 4.5) %>%
...
```{r}
data_height$CC<-data_height$KitDM*0.5

summary_data <- data_height %>%
  group_by(`Sitio específico`) %>%
  summarise(Unique_Codigo_Count = n_distinct(Código))

```

#0.53 is for multiplying the CC (Kg) of 1 plant, to a 530 indviduals stands in Megagrams
```{r}
# Remove outliers where CC > 51
data_height_filtered <- subset(data_height, CC <= 51)

# Calculate the mean predicted survival for the whole filtered data
mean_pred_survival_all_filtered <- mean(data_height_filtered$PredSurv)

# Create a new data frame for Waterboxx® treatment (filtered)
data_waterboxx_filtered <- subset(data_height_filtered, Tratamiento == "Waterboxx®")
mean_pred_survival_waterboxx_filtered <- mean(data_waterboxx_filtered$PredSurv)

# Create a new data frame for Control treatment (filtered)
data_control_filtered <- subset(data_height_filtered, Tratamiento == "Control")
mean_pred_survival_control_filtered <- mean(data_control_filtered$PredSurv)

# Create the plots with smaller and more pale dots, trend curves, and colors
Plot1 <- ggplot(data = data_height_filtered, aes(x = age, y = CC * 0.53 * PredSurv)) +
  geom_point(aes(color = "Original"), alpha = 0.2, size = 0.9, position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.2)) +  # Smaller and more pale dots for the original curve with jitter
  geom_smooth(method = "loess", se = FALSE, color = "grey") +  # Trend line for the original curve
  geom_point(data = data_waterboxx_filtered, aes(x = age, y = CC * 0.53 * PredSurv, color = "Waterboxx"), alpha = 0.2, size = 0.9, position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.2)) +  # Smaller and more pale dots for Waterboxx® with jitter
  geom_smooth(data = data_waterboxx_filtered, aes(x = age, y = CC * 0.53 * PredSurv, color = "Waterboxx"), method = "loess", se = FALSE) +  # Trend line for Waterboxx®
  geom_point(data = data_control_filtered, aes(x = age, y = CC * 0.53 * PredSurv, color = "Control"), alpha = 0.2, size = 0.9, position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.2)) +  # Smaller and more pale dots for Control with jitter
  geom_smooth(data = data_control_filtered, aes(x = age, y = CC * 0.53 * PredSurv, color = "Control"), method = "loess", se = FALSE) +  # Trend line for Control
  labs(x = "Age", y = "CC * Mean Predicted Survival") +
  ggtitle("Curves of (CC * Mean Predicted Survival) vs. Age") +
  scale_color_manual(values = c("Original" = "grey", "Waterboxx" = "orange", "Control" = "purple")) +
  theme_minimal()
...
# Define a function to add Brownian noise to a vector
add_brownian_noise <- function(vector, scale_factor) {
  n <- length(vector)
  delta <- rnorm(n, 0, scale_factor)  # Generate random noise
  return(vector + delta)
}

# Add Brownian noise to the data
data_height_filtered$CC_Noisy <- add_brownian_noise(data_height_filtered$CC * 0.53 * data_height_filtered$PredSurv, scale_factor = 0.2)
data_waterboxx_filtered$CC_Noisy <- add_brownian_noise(data_waterboxx_filtered$CC * 0.53 * data_waterboxx_filtered$PredSurv, scale_factor = 0.2)
data_control_filtered$CC_Noisy <- add_brownian_noise(data_control_filtered$CC * 0.53 * data_control_filtered$PredSurv, scale_factor = 0.2)

# Create the plots with smaller and more pale dots, noisy trend curves, and colors
Plot2 <- ggplot(data = data_height_filtered, aes(x = age, y = CC_Noisy)) +
  geom_point(aes(color = "Original"), alpha = 0.2, size = 0.9, position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.2)) +  # Smaller and more pale dots for the original curve with jitter
  geom_smooth(method = "loess", se = T, color = "#666666") +  # Noisy trend line for the original curve
  geom_point(data = data_waterboxx_filtered, aes(x = age, y = CC_Noisy, color = "Waterboxx"), alpha = 0.2, size = 0.9, position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.2)) +  # Smaller and more pale dots for Waterboxx® with jitter
  geom_smooth(data = data_waterboxx_filtered, aes(x = age, y = CC_Noisy, color = "Waterboxx"), method = "loess", se = T) +  # Noisy trend line for Waterboxx®
  geom_point(data = data_control_filtered, aes(x = age, y = CC_Noisy, color = "Control"), alpha = 0.2, size = 0.9, position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.2)) +  # Smaller and more pale dots for Control with jitter
  geom_smooth(data = data_control_filtered, aes(x = age, y = CC_Noisy, color = "Control"), method = "loess", se = T) +  # Noisy trend line for Control
  labs(x = "Age (years)", y = "Carbon Sequestration (Mg)") +
  ggtitle("Curves of (CC * Mean Predicted Survival with Noise) vs. Age") +
  scale_color_manual(values = c("Original" = "#666666", "Waterboxx" = "orange", "Control" = "purple")) +
  theme_minimal()#+
  #geom_segment(aes(x = 8.75, xend = Inf, y = 22, yend = 22), linetype = "dashed", color = "orange") +
 # geom_segment(aes(x = 8.75, xend = Inf, y = 21, yend = 21), linetype = "dashed", color = "purple") +
  #geom_text(aes(x = 10.25, y = 22.9, label = "22"), color = "orange")+
  #geom_text(aes(x = 10.35, y = 20.2, label = "21"), color = "purple")
...
individual_plant_data <- data.frame(
  PlantID = rep(1:num_plants, each = nrow(data_height_filtered)),
  Age = rep(data_height_filtered$age, times = num_plants),
  PredSurv = rep(data_height_filtered$PredSurv, times = num_plants),
  Tratamiento = rep(data_height_filtered$Tratamiento, times = num_plants),
  CC_Noisy = rep(data_height_filtered$CC_Noisy, times = num_plants)
)

# Add Brownian noise to each individual plant's CC_Noisy
individual_plant_data$CC_Noisy <- add_brownian_noise(individual_plant_data$CC_Noisy * 0.55 * individual_plant_data$PredSurv, scale_factor = 0.2)

# Create a plot with individual plant trajectories
Plot3 <- ggplot(data = individual_plant_data, aes(x = Age, y = CC_Noisy, group = PlantID, color = Tratamiento)) +
  geom_line(alpha = 0.2, size = 0.9) +
  labs(x = "Age", y = "CC * Mean Predicted Survival with Noise") +
  ggtitle("Individual Plant Trajectories of (CC * Mean Predicted Survival with Noise) vs. Age") +
  scale_color_manual(values = c("Waterboxx®" = "orange", "Control" = "purple")) +
  theme_minimal()
...
# Extract predicted probabilities for each treatment group
treatment_levels <- unique(data_height$Tratamiento)
predicted_probabilities <- data.frame()

for (treatment in treatment_levels) {
  subset_data <- subset(data_height, Tratamiento == treatment)
  subset_data$PredSurv <- predict(model_logistic, newdata = subset_data, type = "response")
  predicted_probabilities <- rbind(predicted_probabilities, subset_data)
}

# Create a plot of logistic curves for each treatment group
ggplot(predicted_probabilities, aes(x = age, y = PredSurv, color = Tratamiento)) +
  geom_line(size = 1) +
  labs(x = "Age", y = "Predicted Probability of Estado", color = "Treatment") +
  scale_color_brewer(palette = "Set1") +
  theme_minimal() +
  ggtitle("Logistic Curves of Estado vs. Age by Treatment")
